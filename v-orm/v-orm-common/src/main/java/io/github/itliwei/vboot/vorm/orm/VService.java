...
import io.github.itliwei.vboot.vorm.orm.opt.*;
import io.github.itliwei.vboot.vorm.orm.r.Select;
import io.github.itliwei.vboot.vorm.orm.r.SumOpt;
...
public class VService {
    ...
    private VMapper mapper;

    public VService(VMapper mapper) {
        this.mapper = mapper;
    }
    ...
    public <T extends IdEntity> int save(T entity) {
        if (entity.getId() != null && entity.getId() != 0L) {
            return VOrm.switchM(mapper).insert((Class<IdEntity>) entity.getClass()).obj(entity);
        }else {
            //自增
            return VOrm.switchM(mapper).insert((Class<IdEntity>) entity.getClass()).incId().obj(entity);
        }
    }
    ...
    public <T extends IdEntity> int update(T entity) {
        return VOrm.switchM(mapper).update((Class<IdEntity>) entity.getClass()).obj(entity);
    }
    ...
    public <T extends IdEntity> int update(T entity, boolean useNull) {
        if (useNull) {
            return VOrm.switchM(mapper).update((Class<IdEntity>) entity.getClass()).useNull().obj(entity);
        }
    ...
    public <T extends IdEntity> int update(T entity, Condition... conditions) {
        return VOrm.switchM(mapper).update((Class<IdEntity>) entity.getClass()).entity(entity).where(conditions).exec();
    }
    ...
    public <T extends IdEntity> int update(T entity, boolean useNull, Condition... conditions) {
        Update update = VOrm.switchM(mapper).update((Class<IdEntity>) entity.getClass());
        if (useNull) {
            update.useNull();
        }
    ...
    public <T extends IdEntity> T find(Class<T> clazz, Long id) {
        return (T) VOrm.switchM(mapper).select((Class<IdEntity>) clazz).where(Condition.eq(IdEntity.ID_PN, id)).one();
    }
    ...
    public <T extends IdEntity> T find(Class<T> clazz, Condition... conditions) {
        return (T) VOrm.switchM(mapper).select((Class<IdEntity>) clazz).where(conditions).one();
    }
    ...
    public <T extends IdEntity> T find(Class<T> clazz,QueryModel queryModel) {
        ...
        Select select = VOrm.switchM(mapper).select((Class<IdEntity>) clazz);
    ...
    public <T extends IdEntity> List<T> findAll(Class<T> clazz) {
        return (List<T>) VOrm.switchM(mapper).select((Class<IdEntity>) clazz).list();
    }
    ...
    public <T extends IdEntity> List<T> findList(Class<T> clazz, List<Long> ids) {
        return (List<T>) VOrm.switchM(mapper).select((Class<IdEntity>) clazz)
    ...
    public <T extends IdEntity> int delete(Class<T> clazz, Long id) {
        return VOrm.switchM(mapper).delete((Class<IdEntity>) clazz).where(Condition.eq(IdEntity.ID_PN, id)).exec();
    }
    ...
    public <T extends IdEntity> int delete(Class<T> clazz, List<Long> ids) {
        return VOrm.switchM(mapper).delete((Class<IdEntity>) clazz).where(Condition.in(IdEntity.ID_PN, ids)).exec();
    }
    ...
    public <T extends IdEntity> long count(Class<T> clazz) {
        return VOrm.switchM(mapper).select((Class<IdEntity>) clazz).count().number();
    }
    ...
    public <T extends IdEntity> Number sum(Class<T> clazz, String sumProperty) {
        return VOrm.switchM(mapper).select((Class<IdEntity>) clazz).sum(Field.of(sumProperty)).number();
    }
